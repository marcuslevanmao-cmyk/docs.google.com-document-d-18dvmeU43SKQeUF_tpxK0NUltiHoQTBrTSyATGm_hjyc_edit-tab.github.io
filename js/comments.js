...
const CommentsEngine = (() => {
  ...
  function bindSelectionListener() {
    ...
    // FIX: Clear selection when clicking outside the page, EXCEPT when clicking comment-triggering buttons
    document.addEventListener('mousedown', (e) => {
      if (!e.target.closest('.doc-page') && 
          !e.target.closest('.comment-floating-composer') &&
          !e.target.closest('#add-comment-btn') &&
          !e.target.closest('#sidebar-add-comment-btn')) {
        pendingRange = null;
        reflectAddCommentButtonState();
      }
    });
  ...
  // FIX: Match the actual element ID 'add-comment-btn' from index.html
